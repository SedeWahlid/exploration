...
class QAgent:
    ...
    def __init__(self, grid_size, n_actions=4,alpha=ALPHA, gamma=GAMMA,epsilon=0.00, epsilon_min=0.00, epsilon_decay=0.00):
        # Q-table: one value per (row, col, action). Start optimistic-free at 0.
        self.q_table = np.zeros((grid_size, grid_size, n_actions))
        # Instead of one Q-table, create a list of them
        self.ensemble = [np.zeros((grid_size, grid_size, n_actions)) 
                         for _ in range(grid_size)]
        self.ensemble_size = grid_size
        self.current_ensemble_idx = None

        self.alpha = alpha              # learning rate
        self.gamma = gamma              # discount factor
        self.epsilon = 0.00         # current exploration rate
        self.epsilon_min = 0.00
        self.epsilon_decay = 0.00
        self.n_actions = n_actions
    
    def start_episode(self):
        """Call this at the beginning of every episode in the training loop."""
        # Sample a random hypothesis (Q-table)
        self.current_ensemble_idx = np.random.randint(self.ensemble_size)

    def choose_action(self, state):
        r, c = state
        # Use the specific table sampled for this episode
        q_table = self.ensemble[self.current_ensemble_idx]
        if random.random() < self.epsilon: # keep epsilon optional
            return random.choice(ACTIONS)
        return int(np.argmax(q_table[r, c]))

    def update(self, state, action, reward, next_state, done):
        # Important: In standard Bootstrapped DQN, you update ALL tables 
        # or a random subset. Updating all is easier and works fine here.
        for idx in range(self.ensemble_size):
            q_table = self.ensemble[idx]
            r, c = state
            nr, nc = next_state
            best_next = 0.0 if done else np.max(q_table[nr, nc])
            td_target = reward + self.gamma * best_next
            td_error = td_target - q_table[r, c, action]
            q_table[r, c, action] += self.alpha * td_error
...
def print_policy(env, agent):
    ...
    print("  +" + "---" * env.size + "+")
    # After training, create an averaged Q-table from the ensemble
    agent.q_table = np.mean(agent.ensemble, axis=0)
...
def main():
    ...
    # Animated 2D maze (graphical, not text):
