...
class QAgent:
    ...
    def __init__(self, grid_size, n_actions=4,
        ...
        self.counts = np.zeros((grid_size, grid_size)) + 1e-5  # avoid divide by zero
        self.bonus_coeff = 1.0  # tune this; try 0.5 or 1.0
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        # 1. Increment the count for this state
        self.counts[r, c] += 1
        
        # 2. Compute the exploration bonus (using UCB-style sqrt formula from slide 26)
        #    The less visited, the higher the bonus.
        bonus = self.bonus_coeff / np.sqrt(self.counts[r, c])
        
        # 3. Add the bonus to the raw reward
        augmented_reward = reward + bonus
        
        # 4. Normal Q-Learning update, but using augmented_reward
        nr, nc = next_state
