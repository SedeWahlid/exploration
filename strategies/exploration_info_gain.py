...
class QAgent:
    ...
    def __init__(self, grid_size, n_actions=4,
        ...
        self.model = np.zeros((grid_size, grid_size, n_actions, 2)) 
        self.model_counts = np.zeros((grid_size, grid_size, n_actions)) + 1e-5
        self.info_bonus_coeff = 0.5
    ...
    def update(self, state, action, reward, next_state, done):
        r, c = state
        nr, nc = next_state
        dr, dc = nr - r, nc - c  # actual move

        # 1. Get the model's prediction before updating
        pred_dr, pred_dc = self.model[r, c, action]
        
        # 2. Calculate prediction error (squared distance)
        error = (dr - pred_dr)**2 + (dc - pred_dc)**2
        
        # 3. Intrinsic bonus = error (Slide 44 uses "model error as exploration bonus")
        intrinsic_reward = self.info_bonus_coeff * error
        
        # 4. Update the Q-table with augmented reward
        augmented_reward = reward + intrinsic_reward
        best_next = 0.0 if done else np.max(self.q_table[nr, nc])
        td_target = augmented_reward + self.gamma * best_next
        td_error = td_target - self.q_table[r, c, action]
        self.q_table[r, c, action] += self.alpha * td_error

        # 5. Update the world-model with this new observation (moving average)
        self.model_counts[r, c, action] += 1
        lr = 1.0 / self.model_counts[r, c, action]  # decaying average
        self.model[r, c, action, 0] += lr * (dr - self.model[r, c, action, 0])
        self.model[r, c, action, 1] += lr * (dc - self.model[r, c, action, 1])
